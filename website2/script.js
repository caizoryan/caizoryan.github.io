...
let dom = data.map(
  (a, i) =>
    `<div class="block"><p><span>${i + 1}</span>${
      a.title
    }</p><div class='description'>${a.description}</div></div>`
);
...
function setup() {
  ...
  // colors
  destinationColor = color(255, 144, 40);
  color1 = color(255);
  color2 = color(255);
}
...
function draw() {
  ...
  setGradient(color2, color1);
}

function setGradient(c1, c2) {
  for (let y = 0; y < height; y++) {
    let progress = map(y, 0, height * 1, 0, 1);
